...
public class ChaosConfigModel : PageModel
{
        ...
        int Percentage)
    {
        var config = new ChaosConfig
        {
            ...
            SlowResponseMode = (Models.SlowResponseMode)SlowResponseMode,
            EnableLongStartup = EnableLongStartup,
            CpuDurationMs = Math.Clamp(CpuDurationMs, 10, 30000),
            MemorySizeMb = Math.Clamp(MemorySizeMb, 10, 2000),
            LatencyMs = Math.Clamp(LatencyMs, 10, 60000),
            SlowResponseMs = Math.Clamp(SlowResponseMs, 10, 60000),
            CpuDurationMinMs = Math.Clamp(CpuDurationMinMs, 10, 30000),
            CpuDurationMaxMs = Math.Clamp(CpuDurationMaxMs, 10, 30000),
            MemorySizeMinMb = Math.Clamp(MemorySizeMinMb, 10, 2000),
            MemorySizeMaxMb = Math.Clamp(MemorySizeMaxMb, 10, 2000),
            LatencyMinMs = Math.Clamp(LatencyMinMs, 10, 60000),
            LatencyMaxMs = Math.Clamp(LatencyMaxMs, 10, 60000),
            SlowResponseMinMs = Math.Clamp(SlowResponseMinMs, 10, 60000),
            SlowResponseMaxMs = Math.Clamp(SlowResponseMaxMs, 10, 60000),
            StartupDurationMinMs = Math.Clamp(StartupDurationMinMs, 500, 120000),
            StartupDurationMaxMs = Math.Clamp(StartupDurationMaxMs, 500, 300000),
